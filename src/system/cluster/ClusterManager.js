...
class ClusterManager extends EventEmitter {
	constructor(file, options = {}) {
		...
		if (!file)
			throw new Error("CLIENT_INVALID_OPTION", "File", "specified.");
	...
	async spawn(amount = this.totalShards, delay = 5500, spawnTimeout) {
		...
		this.shardclusterlist = this.shardList.chunk(
		...
		for (let i = 0; i < this.totalClusters; i++) {
			...
			if (delay > 0 && this.clusters.size !== this.totalClusters)
				promises.push(Discord.Util.delayFor(delay));
	...
	_performOnShards(method, args, cluster) {
		if (typeof cluster === "number") {
			if (this.clusters.has(cluster))
				return this.clusers.get(cluster)[method](...args);
			return Promise.reject(
				new Error("CLUSTERING_CLUSTER_NOT_FOUND", cluster)
			);
		}
	...
	async respawnAll(shardDelay = 5000, respawnDelay = 500, spawnTimeout) {
		...
		for (const cluster of this.clusters.values()) {
			...
			if (++s < this.clusters.size && shardDelay > 0)
