		...
		collector.on(
			...
			async button => {
				...
				switch (button.customId) {
					case 'server-avatar':
						if (secondy_embed == null) secondy_embed = new Discord.MessageEmbed(embed_main).setImage(serverAvatar + '?size=1024').setDescription(`${asl2(128)} | ${asl2(256)} | ${asl2(512)} | ${asl2(1024)} | ${asl2(2048)}`);
							...
							components: [
								new Discord.MessageActionRow().addComponents([
									new Discord.MessageButton()
										...
										.setLabel(ctx.t('user_avatar:texts.downloadImage')),
									new Discord.MessageButton().setStyle('SECONDARY').setCustomId('user-avatar').setEmoji('899822412043010088').setLabel(ctx.t('user_avatar:texts.userAvatar')),
								]),
							],
						}).catch(() => {});
						break;
