...
module.exports = class AdvListSubCommand extends Command {
	constructor(client) {
		super(
			{
				name: 'User Adv List',
	...
	async run(ctx) {
		...
		if (!advs.length)
			return ctx.interaction
				.followUp({
					embeds: [this.sendError(ctx.t('adv_list:texts.noWarning'), ctx.author)],
				})
				.catch(() => {});
		...
		async function chunkPage(_index = 0) {
			...
			for (let i = 0; i < chunk[_index].length; i++) {
				...
				const author = (await ctx.client.users.fetch(adv.author).catch(() => {})) || {
			...
			const components = new Discord.MessageActionRow().addComponents([
				new Discord.MessageButton().setEmoji('905602424495026206').setCustomId('back').setStyle('SECONDARY').setDisabled(!_index),
