...
module.exports = class ChannelUnlockSubCommand extends SubCommand {
    ...
    async run(ctx) {
        ...
        if(channel.isVoice()) if(channel.permissionsFor(ctx.guild.roles.everyone).has("CONNECT")) return ctx.interaction.followUp({
            content: ctx.t("channel_unlock:texts.channelNotLocked", {
                channel: channel.toString()
            })
        }).catch(() => {})

        if(channel.isText()) if(channel.permissionsFor(ctx.guild.roles.everyone).has("SEND_MESSAGES")) return ctx.interaction.followUp({
            content: ctx.t("channel_unlock:texts.channelNotLocked", {
                channel: channel.toString()
            })
        }).catch(() => {})
        ...
        ctx.interaction.followUp({
            embeds: [
                ...
                .setDescription(ctx.t("channel_unlock:texts.description", {
                    channel: channel.toString()
                }))
            ]
        }).catch(() => {})
