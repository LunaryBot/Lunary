...
module.exports = class HelpCommand extends Command {
	...
	async run(ctx) {
		...
		const components = new Discord.MessageActionRow().addComponents([new Discord.MessageButton().setStyle('LINK').setURL(invite).setLabel(ctx.t('help:texts.buttons.invite_me')), new Discord.MessageButton().setStyle('LINK').setURL(this.client.config.links.support).setLabel(ctx.t('help:texts.buttons.support')), new Discord.MessageButton().setStyle('LINK').setURL(this.client.config.links.website.home).setLabel(ctx.t('help:texts.buttons.website'))]);
