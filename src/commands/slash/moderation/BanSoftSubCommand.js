...
module.exports = class BanSoftSubCommand extends SubCommand {
	...
	async run(ctx) {
			...
			if (
				...
				return ctx.interaction
					.reply({
						embeds: [
							this.sendError(
								ctx.t("ban_soft:texts.mandatoryReason"),
								ctx.author
							),
						],
					})
		...
		const membro = await ctx.interaction.guild.members
			.fetch(user.id)
			.catch(() => {});
		if (membro) {
			if (!membro.bannable)
		...
		if (!ctx.UserDB.configs.has("QUICK_PUNISHMENT")) {
			await ctx.interaction
				.reply(confirm_punish(ctx, user, reason))
				.catch(() => {});
		...
		async function ban() {
			...
			try {
				if (
					membro &&
					ctx.interaction.options.getBoolean("notify-dm") != false
				)
					await user.send(
						...
						if (user.id != ctx.author.id) {
							if (
								...
								if (
									...
									reason !=
										ctx.t(
											"ban_soft:texts.reasonNotInformed"
										)
								) {
									xp += generateXP();
								}
