...
module.exports = class AdvCommand extends Command {
	 ...
	 */
	 ...
	 async run(ctx) {
		...
		const { replace_placeholders, message_modlogs, calculate_levels } = this.utils;
		...
		async function punishment(activeQuickPunishment) {
				...
				if(punishment_channel.permissionsFor(ctx.client.user.id).has(18432)) {
					...
					punishment_message = replace_placeholders(
							...
							ctx.author, 
							{
								...
								type: ctx.t('adv:texts.punishmentType'),
							}
						)
			...
			if (modlogs_channel && modlogs_channel.permissionsFor(ctx.client.user.id).has(18432))
				modlogs_channel
					.send({
						embeds: [message_modlogs(ctx.author, user.user, reason, 'adv', ctx.t, id, Infinity, attachment?.url)],
						components: [
							new Discord.MessageActionRow()
							.addComponents([
								new Discord.MessageButton()
