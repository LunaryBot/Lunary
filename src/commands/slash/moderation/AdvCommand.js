		...
		if (!reason) {
			if (ctx.GuildDB.configs.has('MANDATORY_REASON') && !ctx.member.botpermissions.has('LUNAR_NOT_REASON'))
				return ctx.interaction
					.reply({
						embeds: [this.sendError(ctx.t('adv:texts.mandatoryReason'), ctx.author)],
					})
					.catch(() => {});
		...
		async function adv() {
			...
			try {
				if (ctx.interaction.options.getBoolean('notify-dm') != false)
					await user.send(
						ctx.t('adv:texts.default_dm_message', {
							emoji: ':hiking_boot:',
			...
			if (channel_modlogs && channel_modlogs.permissionsFor(ctx.client.user.id).has(18432))
				channel_modlogs
					.send({
						embeds: [message_modlogs(ctx.author, user.user, reason, 'adv', ctx.t, id)],
			...
			if (ctx.UserDB.lastPunishmentApplied) {
				if (!user.user.bot) {
					if (user.id != ctx.author.id) {
						if (user.id != ctx.UserDB.lastPunishmentApplied.user || (user.id == ctx.UserDB.lastPunishmentApplied.user && ctx.UserDB.lastPunishmentApplied.type != 4) || (!isNaN(ctx.UserDB.lastPunishmentApplied.date) && user.id == ctx.UserDB.lastPunishmentApplied.user && Date.now() - ctx.UserDB.lastPunishmentApplied.date > 13 * 1000 * 60)) {
