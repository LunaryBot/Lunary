...
module.exports = class AdvInfoSubCommand extends SubCommand {
	...
	async run(ctx) {
		...
		const embed = new Discord.MessageEmbed()
			.setAuthor(
				"Informações sobre a advertência",
				"https://cdn.discordapp.com/emojis/833078041084166164.png?size=128"
			)
			.setColor("#fee75c")
			.addField(
				"<:cry:902169710949445643> Usuário advertido",
				`${user.toString()} (\`${user.tag} - ${adv.user}\`)`
			)
			.addField(
				"<:sigh:885721398788632586> Motivo",
				`\`\`\`${decodeURI(adv.reason)}\`\`\`\n- **${ctx.t(
					"adv_list:texts.punishedBy"
