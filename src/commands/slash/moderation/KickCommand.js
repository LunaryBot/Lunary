...
module.exports = class KickCommand extends Command {
	...
	async run(ctx) {
		...
		if (!reason) {
			if (ctx.GuildDB.configs.has('MANDATORY_REASON') && !ctx.member.botpermissions.has('LUNAR_NOT_REASON'))
				return ctx.interaction
					.reply({
						embeds: [this.sendError(ctx.t('kick:texts.mandatoryReason'), ctx.author)],
					})
					.catch(() => {});
		...
		if (reason > 400)
			return ctx.interaction
				.reply({
					embeds: [this.sendError(ctx.t('kick:texts.veryBigReason'), ctx.author)],
				})
				.catch(() => {});
		...
		async function kick() {
			...
			if (channel_modlogs && channel_modlogs.permissionsFor(ctx.client.user.id).has(18432))
				channel_modlogs
					.send({
						embeds: [message_modlogs(ctx.author, user.user, reason, 'kick', ctx.t, id)],
			...
			if (ctx.UserDB.lastPunishmentApplied) {
				if (!user.user.bot) {
					if (user.id != ctx.author.id) {
						if (user.id != ctx.UserDB.lastPunishmentApplied.user || (user.id == ctx.UserDB.lastPunishmentApplied.user && ctx.UserDB.lastPunishmentApplied.type != 2) || (!isNaN(ctx.UserDB.lastPunishmentApplied.date) && user.id == ctx.UserDB.lastPunishmentApplied.user && Date.now() - ctx.UserDB.lastPunishmentApplied.date > 13 * 1000 * 60)) {
