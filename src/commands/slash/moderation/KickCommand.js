...
module.exports = class KickCommand extends Command {
	...
	async run(ctx) {
		...
		async function punishment(activeQuickPunishment) {
			...
			await user.kick(ctx.t("general:punishedBy", {
			...
			if (modlogs_channel && modlogs_channel.permissionsFor(ctx.client.user.id).has(18432))
				modlogs_channel
					.send({
						embeds: [message_modlogs(ctx.author, user.user, reason, 'kick', ctx.t, id, Infinity, attachment?.url)],
						components: [
							new Discord.MessageActionRow()
							.addComponents([
								new Discord.MessageButton()
				...
				if(!user.bot) {
					if(user.id != ctx.author.id) {
						if(
							...
							|| (user.id == lastPunishmentApplied.user && lastPunishmentApplied.type != 2)
