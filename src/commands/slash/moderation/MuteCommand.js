...
module.exports = class MuteCommand extends Command {
	...
	async run(ctx) {
			...
			if (
				...
				return ctx.interaction
					.reply({
						embeds: [
							this.sendError(
								ctx.t("mute:texts.mandatoryReason"),
								ctx.author
							),
						],
					})
		...
		if (!muterole) {
			...
			if (ctx.me.permissions.has("READ_MESSAGE_HISTORY"))
				muterolePerms.add("READ_MESSAGE_HISTORY");
			if (ctx.me.permissions.has("VIEW_CHANNEL"))
				muterolePerms.add("VIEW_CHANNEL");
			...
			this.client.db
				.ref(`Servers/${ctx.guild.id}/`)
				.update({ muterole: muterole.id });
		}

		if (
			muterole &&
			ctx.guild.roles.cache.get(muterole.id).position >=
				ctx.guild.me.roles.highest.position
		)
			return ctx.interaction
				.reply({
					embeds: [
						this.sendError(
							ctx.t(
								"mute:texts.lunyMissingPermissionsToManagerMuterole"
							),
							ctx.author
						),
					],
		...
		if (!ctx.UserDB.configs.has("QUICK_PUNISHMENT")) {
			await ctx.interaction
				.reply(confirm_punish(ctx, user.user, reason, time))
				.catch(() => {});
		...
		async function mute() {
			if (!highest_position(ctx.me, user))
				return {
					embeds: [
						this.sendError(
							ctx.t(
								"general:texts.lunyMissingPermissionsToPunish"
							),
							ctx.author
						),
					],
					...
					await user.send(
						ctx.t("mute:texts.default_dm_messages_punish", {
							...
							time:
								time != "..."
									? format(time)
									: ctx.t("general:durationNotDetermined"),
						})
					);
			} catch (_) {
				notifyDM = false;
			...
			const roles =
				user.roles.cache
					.filter((x) => !x.managed && x.id != ctx.guild.id)
					.map((x) => x.id) || [];
			...
			if (roles && roles.length)
				await user.roles.remove(roles).catch(() => {});
					...
					if (user.id != ctx.author.id) {
						if (time != "..." && time > 10 * 1000 * 60) {
							if (
								...
								(user.id ==
									ctx.UserDB.lastPunishmentApplied.user &&
									ctx.UserDB.lastPunishmentApplied.type !=
										3) ||
								(!isNaN(
								...
								if (
									reason !=
										ctx.UserDB.lastPunishmentApplied
											.reason &&
									reason !=
										ctx.t("adv:texts.reasonNotInformed")
								) {
									xp += generateXP();
								}
							}
