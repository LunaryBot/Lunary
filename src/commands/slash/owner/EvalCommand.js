...
module.exports = class EvalCommand extends Command {
	...
	async run(ctx) {
			...
			return ctx.interaction.reply({
				embeds: [
					new Discord.MessageEmbed()
						.setColor("RED")
						.setDescription(
							"**Apenas meus desenvolvedores podem usar este comando!**"
						),
				],
			});
		...
		try {
			let result;
			if (ctx.interaction.options.getString("type") == "--bash")
				result = consoleRun(ctx.interaction.options.getString("code"));
			else if (ctx.interaction.options.getString("type") == "--async")
				result = await eval(
					`(async() => { ${ctx.interaction.options.getString(
						"code"
					)} })()`
				);
			...
			let msg = await ctx.interaction.reply({
				content: `\`\`\`js\n${result
					...
					.replace(/```/g, "\\`\\`\\`")
					.slice(0, 1990)}\`\`\``,
				ephemeral: ctx.interaction.options.getBoolean("hide")
					? true
					: false,
			});
		} catch (err) {
