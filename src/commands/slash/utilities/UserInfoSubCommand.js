...
module.exports = class UserInfoSubCommand extends SubCommand {
	...
	async run(ctx) {
		...
		const comp_next = new Discord.MessageActionRow().addComponents([new Discord.MessageButton().setCustomId('next').setStyle('SECONDARY').setEmoji('858168497950818324')]);
					...
					case 'next':
						if (secondy_embed == null) {
							...
							secondy_embed = new Discord.MessageEmbed(base_embed.toJSON())
								.addField(
									`<:Tools:853645102575910963> ${ctx.t('user_info:texts.memberRoles', {
										size: roles.length,
									})}`,
									`> ${
										roles.length == 0
											...
											: `${
													roles.length > 40
														? `${roles
																.slice(0, 40)
																.map(x => `\`${x.length > 47 ? x.slice(0, 44) + '...' : x}\``)
																.join(', ')} e mais ${roles.length - 40} cargo${roles.length - 40 == 1 ? '' : 's'}`
														: roles.map(x => `\`${x.length > 47 ? x.slice(0, 44) + '...' : x}\``).join(', ')
											  }`
									}`,
								)
								.addField(
									`:closed_lock_with_key: ${ctx.t('user_info:texts.memberPermissions', {
										rank_permissions: (() => {
									...
									'> ' +
										member.permissions
											.toArray()
											.map(x => `\`${ctx.t(`permissions:${x}`)}\``)
											.join(', ') || '🙅‍♀️ Esse membro não possui permissões...',
								);
						}
						msg.edit({
