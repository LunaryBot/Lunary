...
module.exports = class UserBannerSubCommand extends SubCommand {
	...
	async run(ctx) {
		...
		if (!user.banner && !user.banner_color)
			return await ctx.interaction
				.reply({
					embeds: [
						this.sendError(
							ctx.t('user_banner:texts.userDoesNotHaveBanner', {
								user: `<@${user.id}>`,
							}),
						),
					],
		...
		const embed = new Discord.MessageEmbed().setAuthor(user.username, `https://cdn.discordapp.com/emojis/${this.client.config.devs.includes(user.id) ? '844347009543569449' : '832083303627620422'}.png?size=128`).setColor(this.client.config.devs.includes(user.id) ? '#FFFAFA' : '#A020F0');
		...
		if (user.banner) {
			...
			data.components = [new Discord.MessageActionRow().addComponents([new Discord.MessageButton().setStyle('LINK').setURL(bannerURL).setLabel(ctx.t('user_banner:texts.downloadImage'))])];
