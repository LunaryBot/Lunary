	...
	async run(ctx) {
		...
		const menu = new Discord.MessageSelectMenu()
			.addOptions(
				results.map(function (anime, i) {
					return {
						label: `${`${anime.attributes.canonicalTitle}`.shorten(
							100
						)}`,
						value: `${i}`,
					};
		...
		ctx.interaction
			.followUp({
				content: `<:Kitsu:854675062498000896> | ${ctx.t(
					"kitsu_anime:texts.chooseAnime",
					{ search: search.shorten(300) }
				)}`,
				components: [
		...
		function createEmbed(data) {
			const embed = new Discord.MessageEmbed()
				...
				.setDescription(
					`> :bookmark: **| ${ctx.t("kitsu_anime:texts.ID")}:** \`${
						data.id
					}\`
            > :books: **| ${ctx.t("kitsu_anime:texts.volumes")}:** \`${
						data.attributes.volumeCount
							? data.attributes.volumeCount
							: ctx.t("kitsu_anime:texts.notFound")
					}\`
            > <:VideoPlayer:854889422927560754> **| ${ctx.t(
				"kitsu_anime:texts.episodes"
			)}:** \`${
            ...
            > :alarm_clock: **| ${ctx.t(
				"kitsu_anime:texts.episodeMinutes"
			)}:** \`${
            ...
            > :calendar_spiral: **| ${ctx.t("kitsu_anime:texts.endedIn")}:** ${
						data.attributes.endDate
							...
							: ctx.t("kitsu_anime:texts.noEned")
					}
            
            > :medal: **| ${ctx.t("kitsu_anime:texts.popularityRank")}:** \`#${
						data.attributes.popularityRank
					}°\`
            > :medal: **| ${ctx.t("kitsu_anime:texts.ratingRank")}:** \`#${
						data.attributes.ratingRank
					}°\`
            > :star: **| ${ctx.t("kitsu_anime:texts.ratingAverage")}:** \`${
						data.attributes.averageRating
					}%\``
				)
				.addField(
					`> :bookmark_tabs: ${ctx.t(
						"kitsu_anime:texts.description"
					)}:`,
					`>>> \`\`\`${
						data.attributes.description
							? `${data.attributes.description}`.shorten(300)
							: ctx.t("kitsu_anime:texts.noDescription")
					}\`\`\``
				);
