...
class UserInfoSubCommand extends SubCommand {
    ...
    public async run(context: IContextInteractionCommand): Promise<any> {
        ...
        const member: Eris.Member = user.id == context.user.id ? context.member : context.options.get('user', { member: true });
        ...
        context.interaction.createMessage({ embeds, components });
...
export default UserInfoSubCommand;