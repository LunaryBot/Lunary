...
const {format_time: { format }, randomCharacters} = require("../../../utils/index")
...
module.exports = class EvalCommand extends Command {
     ...
     */
    async run(ctx) {
        if(!this.client.config.devs.includes(ctx.author.id)) return ctx.message.reply({
            embeds: [
        ...
        if(!ctx.args[0]) return ctx.message.reply({
            embeds: [
                new Discord.MessageEmbed()
                .setColor('RED')
        ...
        try {
            ...
            await ctx.message.reply({
                content: `\`\`\`js\n${`${e}`.replace(this.client.token, "🙃").replace(/```/g, "\\`\\`\\`").slice(0, 1990)}\`\`\``
            })
        }
    } 
}
