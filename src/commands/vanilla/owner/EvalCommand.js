...
module.exports = class EvalCommand extends Command {
	...
	async run(ctx) {
			...
			return ctx.message.reply({
				embeds: [
					new Discord.MessageEmbed()
						.setColor("RED")
						.setDescription(
							"**Você precisa fornecer um código para executar o eval!**"
						)
						.setFooter(
							ctx.author.tag,
							ctx.author.displayAvatarURL({ dynamic: true })
		...
		if (coderegex.test(conteudo))
			conteudo = conteudo.replace(coderegex, "$1");

		const start = Date.now();
		try {
			let result;
			if (evalBash == true) result = consoleRun(conteudo);
			else if (evalAsync == true)
				result = await eval(`(async() => { ${conteudo} })()`);
