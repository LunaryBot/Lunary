...
module.exports = class Locale {
	constructor(locale) {
		this.locale = locale;
		this.dirname =
			__dirname.replace("structures", "locales") + `/${locale}`;
		...
		this.t = (ref, variables) => {
			...
			if (!existsSync(this.dirname + `/${path}`)) {
				if (existsSync(this.dirname + `/commands/${path}`))
					path = this.dirname + `/commands/${path}`;
				else return ":bug:";
			...
			let val =
				typeof data == "object" && !Array.isArray(data)
					? new ObjRef(data, ".").ref(split.slice(1).join(":")).val()
					: data;
