...
module.exports = class MuteEndEvent extends Event {
	...
	async run(data) {
		...
		if (guildDB.configs.has('LOG_UNMUTE')) {
			...
			if (channel_punish && channel_punish.permissionsFor(this.client.user.id).has(18432))
				channel_punish.send({
					embeds: [message_punish(this.client.user, user.user, reason, 'unmute', t, this.client)],
				});
		}

		const channel_modlogs = guild.channels.cache.get(guildDB.chat_modlogs);
		if (channel_modlogs && channel_modlogs.permissionsFor(this.client.user.id).has(18432))
			channel_modlogs.send({
				embeds: [message_modlogs(this.client.user, user.user, reason, 'unmute', t, data.id)],
			});
