	...
	async run(message) {
		...
		const regexp = new RegExp(
			`^(${`${
				message.guild
					? new ObjRef(GuildsDB)
							.ref(`Servidores/${message.guild.id}/prefix`)
							.val() || defaultprefix
					: defaultprefix
			}`.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")}|<@!?${
				this.client.user.id
		...
		try {
			...
			const command = this.client.commands.vanilla.find(
				(c) =>
					c.name == commandName ||
			...
			if (!command) return;
			if (message.guild) {
				const perms = message.channel.permissionsFor(
					this.client.user.id
				);
				if (perms) {
					if (!perms.has("EMBED_LINKS"))
						return message.reply(
							`> Eu preciso de permissão de \`Enviar Links\``
						);
					if (!perms.has("USE_EXTERNAL_EMOJIS"))
						return message.reply(
							`> Eu preciso de permissão de \`Usar Emojis Externos\``
						);
					if (!perms.has("ADD_REACTIONS"))
						return message.reply(
							`> Eu preciso de permissão de \`Adicionar Reações\``
						);
					if (!perms.has("ATTACH_FILES"))
						return message.reply(
							`> Eu preciso de permissão de \`Anexar arquivos\``
						);
				}
			}
			...
			const ctx = new ContextCommand(
				{
					...
					prefix: message.content.replace(
						message.content.replace(regexp, ""),
						""
					),
