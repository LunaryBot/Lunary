...
module.exports = class GuildMemberRemoveEvent extends Event {
  ...
  async run(member) {
    ...
    setTimeout(async() => {
        const guildDB = await this.client.db.ref(`Servers/${member.guild.id}/`).once("value").then(x => new GuildDB(x.val() || {}))
        const roles = member.roles.cache.filter(x => !x.managed && x.id != member.guild.id).map(x => x.id) || []
        const muterole = member.guild.roles.cache.get(guildDB.muterole)
        if(!muterole) return
        if(muterole.position >= member.guild.me.roles.highest.position) return
        const data = {
            user: member.id,
        ...
        if(roles && roles.length) await member.roles.remove(roles).catch(() => {})

        await member.roles.add(muterole.id)
        mutesdb.ref(`${member.guild.id}_${member.id}`).set(data)
        if(time != "...") {
            const timeout = setTimeout(() => this.client.emit("muteEnd", data), time - Date.now())
            this.client.mutes.set(`${member.guild.id}_${member.id}`, timeout)
        }
    }, 1000)
  }
}