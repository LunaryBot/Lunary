...
module.exports = (client) => {
	...
	for (let type of types) {
		...
		for (pasta of pastas) {
			let commands = readdirSync(
				`${__dirname}/../commands/${type}/${pasta}`
			).filter(
				(file) => file.endsWith("Command.js") && !fileRegex.test(file)
			);
			for (command of commands) {
				let base = require(__dirname +
					`/../commands/${type}/${pasta}/${command}`);
