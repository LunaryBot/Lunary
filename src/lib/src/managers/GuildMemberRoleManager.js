...
class GuildMemberRoleManager extends DataManager {
	...
	get premiumSubscriberRole() {
		return (
			this.cache.find((role) => role.tags?.premiumSubscriberRole) ?? null
		);
	}
	...
	get botRole() {
		...
		return (
			this.cache.find(
				(role) => role.tags?.botId === this.member.user.id
			) ?? null
		);
	}
	...
	async add(roleOrRoles, reason) {
		if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
			...
			const newRoles = [
				...new Set(resolvedRoles.concat(...this.cache.keys())),
			];
	...
	async remove(roleOrRoles, reason) {
		if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
			...
			await this.client.api.guilds[this.guild.id].members[
				this.member.id
			].roles[roleOrRoles].delete({ reason });

			const clone = this.member._clone();
			const newRoles = this.cache.filter(
				(role) => role.id !== roleOrRoles
			);
