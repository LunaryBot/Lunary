...
class ThreadManager extends CachedManager {
	...
	async create({
		...
		if (startMessage) {
			const startMessageId =
				this.channel.messages.resolveId(startMessage);
			if (!startMessageId)
				throw new TypeError(
					"INVALID_TYPE",
					"startMessage",
		...
		if (autoArchiveDuration === "MAX") {
			...
			if (
				this.channel.guild.features.includes("SEVEN_DAY_THREAD_ARCHIVE")
			) {
				autoArchiveDuration = 10080;
			} else if (
				this.channel.guild.features.includes("THREE_DAY_THREAD_ARCHIVE")
			) {
	 ...
	 */
	async fetchArchived(
		{ type = "public", fetchAll = false, before, limit } = {},
		cache = true
	) {
		let path = this.client.api.channels(this.channel.id);
		...
		if (typeof before !== "undefined") {
			if (
				before instanceof ThreadChannel ||
				/^\d{16,19}$/.test(String(before))
			) {
