...
class GuildManager extends CachedManager {
	...
	async create(
		...
		for (const channel of channels) {
			...
			for (const overwrite of channel.permissionOverwrites) {
				...
				overwrite.allow &&= Permissions.resolve(
					overwrite.allow
				).toString();
				overwrite.deny &&= Permissions.resolve(
					overwrite.deny
				).toString();
			}
		...
		for (const role of roles) {
			...
			role.permissions &&= Permissions.resolve(
				role.permissions
			).toString();
		}
	...
	async fetch(options = {}) {
		...
		return data.reduce(
			(coll, guild) =>
				coll.set(guild.id, new OAuth2Guild(this.client, guild)),
			new Collection()
		);
	}
}
