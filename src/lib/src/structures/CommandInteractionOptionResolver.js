...
class CommandInteractionOptionResolver {
	...
	_getTypedOption(name, type, properties, required) {
		...
		if (!option) {
			...
			properties.every(
				(prop) =>
					option[prop] === null || typeof option[prop] === "undefined"
			)
		) {
			throw new TypeError(
	...
	getFocused(required = false) {
		const option = this._hoistedOptions.find((o) => o.focused);
		if (!option && required)
			throw new Error("Any focused option found but is required!");
		return option;
	}
