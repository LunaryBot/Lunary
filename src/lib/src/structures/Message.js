...
class Message extends Base {
	...
	get emojis() {
		...
		for (let match of this.content.matchAll(
			...
			const data = this.client.emojis.cache.get(id) || {
				...
				id,
				name,
				url: this.client.rest.cdn.Emoji(
					id,
					Boolean(animated) ? "gif" : "png"
				),
			};

			const emoji = new Emoji(this.client, data);
			if (this._emojis.get(emoji.id))
				this._emojis.set(
					emoji.id +
						"~" +
						Number(
							this._emojis.filter((x) => x.id == emoji.id).size +
								1
						),
					emoji
				);
	...
	edit(options) {
		if (!this.channel)
			return Promise.reject(new Error("CHANNEL_NOT_CACHED"));
		...
		if (options instanceof MessagePayload) {
			...
			data = MessagePayload.create(this, options, {
				reply: {
					...
					failIfNotExists:
						options?.failIfNotExists ??
						this.client.options.failIfNotExists,
				},
			});
		}
		...
		if (equal && rawData) {
			equal =
				...
				this.editedTimestamp ===
					new Date(rawData.edited_timestamp).getTime();
		}
