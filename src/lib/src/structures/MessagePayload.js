...
class MessagePayload {
	...
	get isWebhook() {
		...
		return (
			this.target instanceof Webhook ||
			this.target instanceof WebhookClient
		);
	}
	...
	get isUser() {
		...
		return (
			this.target instanceof User || this.target instanceof GuildMember
		);
	}
	...
	async resolveFiles() {
		...
		this.files = await Promise.all(
			this.options.files?.map((file) =>
				this.constructor.resolveFile(file)
			) ?? []
		);
		return this;
	}
