	...
	makeCache: Options.cacheWithLimits({
		...
		ThreadManager: {
			...
			sweepFilter: LimitedCollection.filterByLifetime({
				getComparisonTimestamp: (x: ThreadChannel) =>
					x.archiveTimestamp ?? 0,
...
client.on("ready", async () => {
	...
	const globalCommand = await client.application?.commands.fetch(
		globalCommandId
	);
	const guildCommandFromGlobal = await client.application?.commands.fetch(
		guildCommandId,
		{ guildId: testGuildId }
	);
	const guildCommandFromGuild = await client.guilds.cache
		.get(testGuildId)
		?.commands.fetch(guildCommandId);

	// @ts-expect-error
	await client.guilds.cache
		.get(testGuildId)
		?.commands.fetch(guildCommandId, { guildId: testGuildId });

	// Test command permissions
	const globalPermissionsManager = client.application?.commands.permissions;
	const guildPermissionsManager =
		client.guilds.cache.get(testGuildId)?.commands.permissions;
	const originalPermissions =
		await client.application?.commands.permissions.fetch({
			guild: testGuildId,
		});
	...
	await globalPermissionsManager?.has({
		command: globalCommandId,
		guild: testGuildId,
		...
		fullPermissions: [
			{
				id: globalCommandId,
				permissions: [
					{ type: "ROLE", id: testGuildId, permission: true },
				],
			},
		],
	});
	...
	await guildPermissionsManager?.has({
		command: globalCommandId,
		permissionId: testGuildId,
	});
	...
	await guildCommandFromGlobal?.permissions.has({
		permissionId: testGuildId,
	});
	...
	// @ts-expect-error
	await guildCommandFromGuild?.permissions.has({
		command: guildCommandId,
		permissionId: testGuildId,
	});
	// @ts-expect-error
	...
	client.application?.commands.permissions.set({
		guild: testGuildId,
		fullPermissions:
			originalPermissions?.map((permissions, id) => ({
				permissions,
				id,
			})) ?? [],
	});
...
client.on("guildCreate", (g) => {
	...
	assertIsPromiseMember(
		g.members.add(testUserId, { accessToken: "totallyRealAccessToken" })
	);
...
client.on("messageCreate", (message) => {
	...
	assertType<Promise<ButtonInteraction>>(
		message.awaitMessageComponent({ componentType: "BUTTON" })
	);
	assertType<Promise<ButtonInteraction>>(
		channel.awaitMessageComponent({ componentType: "BUTTON" })
	);
	...
	assertType<Promise<SelectMenuInteraction>>(
		message.awaitMessageComponent({ componentType: "SELECT_MENU" })
	);
	assertType<Promise<SelectMenuInteraction>>(
		channel.awaitMessageComponent({ componentType: "SELECT_MENU" })
	);
	assertType<InteractionCollector<SelectMenuInteraction>>(
		selectMenuCollector
	);
	...
	assertType<Promise<MessageComponentInteraction>>(
		message.awaitMessageComponent()
	);
	assertType<Promise<MessageComponentInteraction>>(
		channel.awaitMessageComponent()
	);
	assertType<InteractionCollector<MessageComponentInteraction>>(
		defaultCollector
	);

	// Verify that additional options don't affect default collector types.
	const semiDefaultCollector = message.createMessageComponentCollector({
		time: 10000,
	});
	assertType<InteractionCollector<MessageComponentInteraction>>(
		semiDefaultCollector
	);
	const semiDefaultCollectorChannel = message.createMessageComponentCollector(
		{ time: 10000 }
	);
	assertType<InteractionCollector<MessageComponentInteraction>>(
		semiDefaultCollectorChannel
	);

	// Verify that interaction collector options can't be used.

	// @ts-expect-error
	const interactionOptions = message.createMessageComponentCollector({
...
client.on("interaction", async (interaction) => {
	...
	assertType<GuildMember | APIInteractionGuildMember | null>(
		interaction.member
	);
...
declare const notPropertyOf: <T, P extends PropertyKey>(
	value: T,
	property: P & Exclude<P, keyof T>
) => void;
...
{
	type ApplicationCommandScope = ApplicationCommand<{
		guild: GuildResolvable;
	}>;

	assertType<Promise<ApplicationCommandScope>>(
		applicationCommandManager.create(applicationCommandData)
	);
	assertType<Promise<ApplicationCommand>>(
		applicationCommandManager.create(applicationCommandData, "0")
	);
	assertType<Promise<ApplicationCommandScope>>(
		applicationCommandManager.edit(
			applicationCommandResolvable,
			applicationCommandData
		)
	);
...
declare const applicationChoiceOptionData: ApplicationCommandOptionData & {
...
declare const applicationSubGroupCommandData: ApplicationCommandSubGroupData;
{
	...
	assertType<ApplicationCommandSubCommandData[] | undefined>(
...
{
	assertType<"SUB_COMMAND" | ApplicationCommandOptionTypes.SUB_COMMAND>(
		applicationSubCommandData.type
	);
...
assertType<Promise<Collection<Snowflake, ApplicationCommand>>>(
	guildApplicationCommandManager.fetch(undefined, {})
);
assertType<Promise<ApplicationCommand>>(
	guildApplicationCommandManager.fetch("0")
);
...
declare const guildChannelManager: GuildChannelManager;
{
	...
	assertType<Promise<VoiceChannel>>(
		guildChannelManager.create("name", { type: "GUILD_VOICE" })
	);
	assertType<Promise<CategoryChannel>>(
		guildChannelManager.create("name", { type: "GUILD_CATEGORY" })
	);
	assertType<Promise<TextChannel>>(
		guildChannelManager.create("name", { type: "GUILD_TEXT" })
	);
	assertType<Promise<NewsChannel>>(
		guildChannelManager.create("name", { type: "GUILD_NEWS" })
	);
	assertType<Promise<StoreChannel>>(
		guildChannelManager.create("name", { type: "GUILD_STORE" })
	);
	assertType<Promise<StageChannel>>(
		guildChannelManager.create("name", { type: "GUILD_STAGE_VOICE" })
	);

	assertType<Promise<Collection<Snowflake, AnyChannel>>>(
...
assertType<Promise<Collection<Snowflake, Role>>>(
	roleManager.fetch(undefined, {})
);
assertType<Promise<Role | null>>(roleManager.fetch("0"));

declare const guildEmojiManager: GuildEmojiManager;
assertType<Promise<Collection<Snowflake, GuildEmoji>>>(
	guildEmojiManager.fetch()
);
assertType<Promise<Collection<Snowflake, GuildEmoji>>>(
	guildEmojiManager.fetch(undefined, {})
);
...
client.on("messageReactionAdd", async (reaction) => {
	...
	if (reaction.message.partial)
		return assertType<string | null>(reaction.message.content);
...
client.on("interactionCreate", async (interaction) => {
	...
	if (interaction.isCommand()) {
		if (interaction.inRawGuild()) {
			...
			assertType<CommandInteraction>(interaction);
			assertType<Promise<Message | APIMessage>>(
				interaction.reply({ fetchReply: true })
			);
		}
		...
		assertType<readonly CommandInteractionOption[]>(
		...
		assertType<CommandInteractionOption[] | undefined>(
			requiredOption.options
		);

		assertType<string | null>(
			interaction.options.getString("name", booleanValue)
		);
		...
		assertType<string | null>(
			interaction.options.getSubcommand(booleanValue)
		);
		...
		assertType<string | null>(
			interaction.options.getSubcommandGroup(booleanValue)
		);
		assertType<string | null>(
