...
export default class PingVanillaCommand extends VanillaCommand {
	...
	async run(context: VanillaCommandContext) {
		...
		const messageContent = `\n**:ping_pong:•Pong!**\n**:satellite_orbital: | Shard:** ${Number(!context.guild ? 0 : context.guild.shard.id)} - [<:rocket:1019957255233355807> Cluster ${Number(this.lunary.cluster.clusterId) + 1}]\n**⏰ | Gateway Ping:** \`${Math.floor(shards.reduce((a,b) => a + b.latency, 0) / shards.size)}ms\`\n**⚡ | API Ping:**`
		const ping = Date.now()
	...
	async runPingClusters(context: VanillaCommandContext) {
		const stats = (await this.lunary.cluster.eval(`
                    const shards = this.lunary.shards
