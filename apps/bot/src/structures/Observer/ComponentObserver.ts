...
export class ComponentSubscription extends Subscription<ComponentSubscriptionCallbackContext> {
  ...
  constructor(observer: ComponentObserver, options: ComponentSubscriptionOptions) {
    super(observer as any as Observer<ComponentSubscriptionCallbackContext>)
...
export class ComponentObserver {
  ...
  subscribe(options: ComponentSubscriptionOptions) {
    const subscription = new ComponentSubscription(this, options)

    subscription.setCallback(options.callback)
